...
// Остаток по счёту за всё время: приход плюс, расход и изъятие минус
const balanceOf = (entries, acctId) =>
  entries
    .filter((e) => e.acct === acctId)
    .reduce((s, e) => s + (e.kind === "in" ? e.amount : -e.amount), 0);
...
function KassaApp() {
  ...
  const push = async (silent = false) => {
    ...
    if (!silent) setSync({ state: "run", msg: "Синхронизирую…" });
  ...
  useEffect(() => {
    ...
    const stale = !data.lastPull || Date.now() - data.lastPull > 6 * 3600000;
    if (stale) pull(true);
  }, [data]);
  ...
  useEffect(() => {
    ...
    const backoff = [3000, 15000, 60000, 300000];
    const wait = backoff[Math.min(fails.current, backoff.length - 1)];
  ...
  useEffect(() => {
    const onVis = () => {
      ...
      if (data) {
        ...
        if (data.sheetUrl && (!data.lastPull || Date.now() - data.lastPull > 6 * 3600000))
          pull(true);
      }
    };
  ...
  const fix = (acctId, actual) => {
    const diff = Math.round(Number(actual) - balanceOf(data.entries, acctId));
    if (!diff) return 0;
    const kind = diff > 0 ? "in" : "out";
    const cat = FIX_CAT;
    const entry = {
      id: "e" + Date.now() + Math.random().toString(36).slice(2, 6),
      ts: Date.now(),
            ...
            {data.sheetUrl && (
              <button className="k-pill" onClick={syncNow}>
                ...
                {sync.state === "run"
                  ? "Синхр."
                  : pending
                  ? (navigator.onLine === false ? "Ждёт сети " : "Таблица ") + pending
                  : "Таблица"}
              </button>
            )}
            <button
...
function HistoryBlock({ data, acctName, acctColor, onDelete, onEdit, onPull, syncing }) {
...
function AccountsBlock({ data, byAcct, acctColor, onFix }) {
  const [open, setOpen] = useState(null);
  const [val, setVal] = useState("");
  const orphan = data.entries.filter((e) => !e.acct);
  const orphanSum = orphan.reduce((s, e) => s + (e.kind === "in" ? e.amount : -e.amount), 0);
  ...
  return (
    <>
      <div style={S.blockHead}>Счета</div>
      <ul style={S.list}>
        {data.accounts.map((a) => {
          const bal = balanceOf(data.entries, a.id);
          const isOpen = open === a.id;
          const diff = val === "" ? null : Math.round(Number(val) - bal);
          return (
            <li key={a.id} style={S.fixRow}>
              <button className="k-acctline" onClick={() => pick(a.id)}>
                <span className="k-dot" style={{ background: acctColor(a.id) }} />
                <span style={{ ...S.rowCat, flex: 1 }}>{a.name}</span>
                <span style={S.acctBal}>{money(bal)}</span>
              ...
              {isOpen && (
                <div style={{ paddingTop: 10 }}>
                  <div style={S.fixTop}>
                    <span style={{ flex: 1, color: MUTED, fontSize: 13 }}>
                      Пришло за месяц
                    </span>
                    <span style={S.rowSum}>{money(byAcct[a.id] || 0)}</span>
                  </div>
                  <div style={S.addRow}>
                    <input
                      className="k-input"
                      inputMode="numeric"
                      placeholder="Сколько на самом деле"
                      value={val}
                  ...
                  {diff !== null && diff !== 0 && (
                    <div style={{ ...S.hint, marginTop: 6 }}>
                      {diff > 0
                        ? "Не записано " + money(diff) + " — добавлю корректировкой"
                        : "Потрачено " + money(-diff) + " — спишу корректировкой"}
                    </div>
                  )}
                </div>
              )}
        ...
        {orphan.length > 0 && (
          <li style={S.fixRow}>
            <div className="k-acctline">
              <span className="k-dot" style={{ background: "#B6BCB6" }} />
              <span style={{ ...S.rowCat, flex: 1, color: MUTED }}>
                Без счёта · {orphan.length}
              </span>
              <span style={{ ...S.acctBal, color: MUTED }}>{money(orphanSum)}</span>
            </div>
          </li>
        )}
      </ul>
      <p style={S.hint}>
        Впишите, сколько на счёте на самом деле. Разница уйдёт в журнал строкой
        «Корректировка» и поправит остаток. В доход и расход месяца она не попадает —
        показана отдельной цифрой выше.
        {orphan.length > 0 &&
          " Строки «без счёта» пришли из таблицы, и карта в них не указана — в остатках они не учтены."}
      </p>
    </>
  );
}

function StatsView({ data, month, setMonth, acctName, acctColor, onFix, history }) {
  ...
  const isFix = (e) => e.cat === FIX_CAT || e.note === FIX_NOTE;
  const ins = rows.filter((e) => e.kind === "in" && !isFix(e));
  const inc = ins.reduce((s, e) => s + e.amount, 0);
  const out = rows
    .filter((e) => e.kind === "out" && !isFix(e))
  ...
  const prevInc = data.entries
    .filter((e) => monthKey(e.ts) === prevKey && e.kind === "in" && !isFix(e))
    .reduce((s, e) => s + e.amount, 0);
    ...
    <>
      ...
      <div style={S.totals}>
        ...
        <div>
          ...
          <div style={S.totalSum}>{money(inc - out + fixNet)}</div>
        </div>
      </div>
      ...
      <AccountsBlock data={data} byAcct={byAcct} acctColor={acctColor} onFix={onFix} />
...
const S = {
  ...
  fixTop: { display: "flex", alignItems: "center", gap: 9, fontSize: 14 },
  acctBal: {
    fontSize: 15,
    fontWeight: 700,
    fontVariantNumeric: "tabular-nums",
